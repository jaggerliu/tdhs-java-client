...
public class HandlerSocketTest {
	...
	private String table ="travelrecord";
	private String fieldList = "id,user_id,traveldate,fee,days";
...
//	@Test
	...
	public static void main(String[] args) throws Exception {
		HandlerSocketTest hst = new HandlerSocketTest();
		hst.setUp();
		hst.testHandlerSocketFind2();
	}
	public void testHandlerSocketFind2() throws IOException{
		...
		try{
			...
			hs.open(host, port);
			System.out.println("open index.");
			hs.command().openIndex("2", db, table, "traveldate_index", fieldList);
			System.out.println("operations.");
			for(int i = 0 ; i <= 1 ; i++){
				if(i != 0 && i % 1 == 0){
					System.out.println("\tcommand size : " + hs.getCommandSize() + "byte");
					...
					if(verbose){
						System.out.println("result.");
						for(HandlerSocketResult result : results){
							System.out.println("count="+result.getMessages().size());
							System.out.println("\t" + result.toString());
						}
					}
					if(i == size)
				...
				String[] keys = new String[]{"2001-02-11"};
				hs.command().find("2", keys,">","1","100000");
